...
void P_InitSwitchList(void)
{
    ...
    {
	alphSwitchList = cacheLumpName<switchlist_t*>("SWITCHES", PU_STATIC);
    }
    else
    {
	...
	if (slindex + 1 >= maxswitches)
	{
	    ...
	    switchlist = static_cast<int*>(I_Realloc(switchlist, newmax * sizeof(*switchlist)));
	    maxswitches = newmax;
	}
    ...
    buttonlist = static_cast<button_t*>(I_Realloc(nullptr, sizeof(*buttonlist) * (maxbuttons = MAXBUTTONS)));
  ...
  int		time )
{
	...
	buttonlist = static_cast<button_t*>(I_Realloc(buttonlist, sizeof(*buttonlist) * maxbuttons));
