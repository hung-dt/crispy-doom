...
void SB_Init(void)
{
    ...
    PatchLTFACE = cacheLumpName<patch_t*>(DEH_String("LTFACE"), PU_STATIC);
    PatchRTFACE = cacheLumpName<patch_t*>(DEH_String("RTFACE"), PU_STATIC);
    PatchBARBACK = cacheLumpName<patch_t*>(DEH_String("BARBACK"), PU_STATIC);
    PatchINVBAR = cacheLumpName<patch_t*>(DEH_String("INVBAR"), PU_STATIC);
    PatchCHAIN = cacheLumpName<patch_t*>(DEH_String("CHAIN"), PU_STATIC);
    if (deathmatch)
    {
        PatchSTATBAR = cacheLumpName<patch_t*>(DEH_String("STATBAR"), PU_STATIC);
    }
    else
    {
        PatchSTATBAR = cacheLumpName<patch_t*>(DEH_String("LIFEBAR"), PU_STATIC);
    }
    if (!netgame)
    {                           // single player game uses red life gem
        PatchLIFEGEM = cacheLumpName<patch_t*>(DEH_String("LIFEGEM2"), PU_STATIC);
    }
    else
    {
        PatchLIFEGEM = cacheLumpNum<patch_t*>(W_GetNumForName(DEH_String("LIFEGEM0"))
                                      + consoleplayer, PU_STATIC);
    }
    PatchLTFCTOP = cacheLumpName<patch_t*>(DEH_String("LTFCTOP"), PU_STATIC);
    PatchRTFCTOP = cacheLumpName<patch_t*>(DEH_String("RTFCTOP"), PU_STATIC);
    PatchSELECTBOX = cacheLumpName<patch_t*>(DEH_String("SELECTBOX"), PU_STATIC);
    PatchINVLFGEM1 = cacheLumpName<patch_t*>(DEH_String("INVGEML1"), PU_STATIC);
    PatchINVLFGEM2 = cacheLumpName<patch_t*>(DEH_String("INVGEML2"), PU_STATIC);
    PatchINVRTGEM1 = cacheLumpName<patch_t*>(DEH_String("INVGEMR1"), PU_STATIC);
    PatchINVRTGEM2 = cacheLumpName<patch_t*>(DEH_String("INVGEMR2"), PU_STATIC);
    PatchBLACKSQ = cacheLumpName<patch_t*>(DEH_String("BLACKSQ"), PU_STATIC);
    PatchARMCLEAR = cacheLumpName<patch_t*>(DEH_String("ARMCLEAR"), PU_STATIC);
    PatchCHAINBACK = cacheLumpName<patch_t*>(DEH_String("CHAINBACK"), PU_STATIC);
    startLump = W_GetNumForName(DEH_String("IN0"));
    for (i = 0; i < 10; i++)
    {
        PatchINumbers[i] = cacheLumpNum<patch_t*>(startLump + i, PU_STATIC);
    }
    PatchNEGATIVE = cacheLumpName<patch_t*>(DEH_String("NEGNUM"), PU_STATIC);
...
static void DrINumber(signed int val, int x, int y)
{
    ...
    if (val < 0)
    {
        if (val < -9)
        {
            V_DrawPatch(x + 1, y + 1, cacheLumpName<patch_t*>(DEH_String("LAME"), PU_CACHE));
        }
        else
        {
            val = -val;
...
void SB_Drawer(void)
{
    ...
    else
    {
        if (SB_state == -1)
        {
            ...
            {
                V_DrawPatch(16, 167,
                            cacheLumpName<patch_t*>(DEH_String("GOD1"), PU_CACHE));
                V_DrawPatch(287, 167,
                            cacheLumpName<patch_t*>(DEH_String("GOD2"), PU_CACHE));
            }
            oldhealth = -1;
        }
        DrawCommonBar();
    ...
    if (CPlayer->powers[pw_flight])
    {
            ...
            || !(CPlayer->powers[pw_flight] & 16))
        {
            ...
            if (CPlayer->mo->flags2 & MF2_FLY)
            {
                ...
                {
                    V_DrawPatch(20, 17, cacheLumpNum<patch_t*>(spinflylump + 15,
                                                       PU_CACHE));
                }
                else
                {
                    V_DrawPatch(20, 17, cacheLumpNum<patch_t*>(spinflylump + frame,
                                                       PU_CACHE));
    ...
    if (CPlayer->powers[pw_weaponlevel2] && !CPlayer->chickenTics)
    {
            ...
            || !(CPlayer->powers[pw_weaponlevel2] & 16))
        {
            frame = (leveltime / 3) & 15;
            V_DrawPatch(300, 17,
                        cacheLumpNum<patch_t*>(spinbooklump + frame, PU_CACHE));
            BorderTopRefresh = true;
            UpdateState |= I_MESSAGES;
        }
        else
...
/*
		...
		{
			V_DrawPatch(291, 0, cacheLumpName<patch_t*>("ARTIPWBK", PU_CACHE));
		}
		else
		{
			BorderTopRefresh = true;
...
void DrawMainBar(void)
{
    ...
    if (ArtifactFlash)
    {
        ...
        V_DrawPatch(182, 161, cacheLumpNum<patch_t*>(temp, PU_CACHE));
        ArtifactFlash--;
             ...
             || oldartiCount != CPlayer->inventory[inv_ptr].count)
    {
        ...
        {
            V_DrawPatch(179, 160,
                        cacheLumpName<patch_t*>(DEH_String(patcharti[CPlayer->readyArtifact]),
                                        PU_CACHE));
    ...
    if (oldkeys != playerkeys)
    {
        ...
        {
            V_DrawPatch(153, 164, cacheLumpName<patch_t*>(DEH_String("ykeyicon"), PU_CACHE));
        }
        if (CPlayer->keys[key_green])
        {
            V_DrawPatch(153, 172, cacheLumpName<patch_t*>(DEH_String("gkeyicon"), PU_CACHE));
        }
        if (CPlayer->keys[key_blue])
        {
            V_DrawPatch(153, 180, cacheLumpName<patch_t*>(DEH_String("bkeyicon"), PU_CACHE));
        }
        oldkeys = playerkeys;
        UpdateState |= I_STATBAR;
    }
    ...
    if (oldammo != temp || oldweapon != CPlayer->readyweapon)
    {
        ...
        if (temp && CPlayer->readyweapon > 0 && CPlayer->readyweapon < 7)
        {
            ...
            V_DrawPatch(111, 172,
                        cacheLumpName<patch_t*>(DEH_String(ammopic[CPlayer->readyweapon - 1]),
                                        PU_CACHE));
        }
        oldammo = temp;
...
void DrawInventoryBar(void)
{
    ...
    for (i = 0; i < 7; i++)
    {
            ...
            && CPlayer->inventory[x + i].type != arti_none)
        {
            ...
            V_DrawPatch(50 + i * 31, 160, cacheLumpName<patch_t*>(patch, PU_CACHE));
...
void DrawFullScreenStuff(void)
{
    ...
    if (!inventory)
    {
        if (CPlayer->readyArtifact > 0)
        {
            ...
            V_DrawTLPatch(286, 170, cacheLumpName<patch_t*>(DEH_String("ARTIBOX"), PU_CACHE));
            V_DrawPatch(286, 170, cacheLumpName<patch_t*>(patch, PU_CACHE));
    ...
    else
    {
        ...
        for (i = 0; i < 7; i++)
        {
            V_DrawTLPatch(50 + i * 31, 168,
                          cacheLumpName<patch_t*>(DEH_String("ARTIBOX"), PU_CACHE));
            if (CPlayer->inventorySlotNum > x + i
                && CPlayer->inventory[x + i].type != arti_none)
            {
                patch = DEH_String(patcharti[CPlayer->inventory[x + i].type]);
                V_DrawPatch(50 + i * 31, 168,
                            cacheLumpName<patch_t*>(patch, PU_CACHE));
