...
static char *GetRegistryString(registry_value_t *reg_val)
{
    ...
    if (RegQueryValueEx(key, reg_val->value,
                        nullptr, &valtype, nullptr, &len) == ERROR_SUCCESS
     && valtype == REG_SZ)
    {
...
// file, returning the full path to the IWAD if found, or nullptr
// if not found.
...
// Returns the location of the IWAD if found, otherwise nullptr.
...
const iwad_t **D_FindAllIWADs(int mask)
{
    ...
    auto result = createStruct<iwad_t const*[arrlen(iwads) + 1]>();
    result_len = 0;
