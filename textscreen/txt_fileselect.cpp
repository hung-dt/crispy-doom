...
char *TXT_SelectFile(const char *window_title, const char **extensions)
{
    ...
    argv = static_cast<char**>(calloc(5 + NumExtensions(extensions), sizeof(char *)));
...
static int DoSelectFile(txt_fileselect_t *fileselect)
{
    ...
    if (TXT_CanSelectFiles())
    {
        ...
        var = static_cast<char**>(fileselect->inputbox->value);
        free(*var);
        *var = path;
        return 1;
    }
...
{
    auto* fileselect = createStruct<txt_fileselect_t>();
